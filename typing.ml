...
let ty_binop op t1 t2  = match op with
  | Plus | Minus | Times | Div -> (match t1, t2 with
              TyInt, TyInt -> TyInt
            | _ -> raise (Err "Typing Error: needs integer type"))
  | And | Or -> (
                    match t1, t2 with
                      TyBool, TyBool -> TyBool
                    | _ -> raise (Err "Typing Error: needs boolean type")
  )
...
let rec  ty_exp exp tyenv = match exp with
    Var id -> (try Environment.lookup id tyenv  with Err e -> raise (Err ("Typing Error "^ e ^ " : " ^ id)))
  | IntV _ -> TyInt
  | BoolV _ -> TyBool
                        ...
                        (
                          match arg1 with
                            TyBool -> let arg2 = ty_exp e2 tyenv in
                                       ...
                                       else raise (Err "Typing Error: then part, else part should have the same type")
                          | _ -> raise (Err "Typing Error: if part should boolean type")
                        )
...
and ty_anddecl exp tyenv  = match exp with
  ...
  | CompAndDecl (id, e1,e2) -> let v = ty_exp e1 tyenv in
                               ...
                               (id,v, Environment.extend id v newtyenv)
